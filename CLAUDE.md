...
**M9.3 state — M9.3a DONE (Style Checker), M9.3b (Wardrobe) is next.** The chunk plan: **a)** Style Checker headline+lens ✓ · **b)** Wardrobe (wearMode live toggles §27.6, as-target-sees-it disguise preview reusing the observer picker, headline strip) · **c)** GM Dashboard (GM Overrides UI §14.3 — pin reads on actor flags, clearly badged + consumed by the pipeline; layout pass; Help tab) · **d)** lookbook card §16.4 + player↔player reads §16.3 · **e)** GM Config (faction matrix editor §14.7, brand house pages §13) + Help tabs everywhere.
