...
**M8 RELEASED (v0.1.0 live on GitHub; CLI-verified; in-Foundry fresh-world check + README screenshots still pending) — M9 IN PROGRESS: M9.1 DONE (gate 246/246), M9.2 CODE-COMPLETE (gate 265/265; in-world check pending), M9.3 (app-by-app rework) is next (see the M9 plan below).** M0–M7 are complete and verified in-world:
...
**M9.3 kickoff (the standing M3–M7 polish agreement + missing surfaces):** rework each app's layout one at a time, leading with the **headline read** (§25.2: one plain-language line + the three hero visuals, everything else collapsible — components already exist from M9.2). Missing surfaces to land alongside: **lookbook card** (§16.4), **GM Overrides UI** (§14.3, pin reads on actor flags), **observer picker** (§24 — generic district / faction / specific-token observers; this is also where the M9.1-deferred view-gating of CPR-native genre counts + the perception lens lands), **wearMode live toggles** (Wardrobe), **brand house pages** (§13), **faction matrix editor** (§14.7), and Help tabs on the remaining apps (the glossary partial is shared — Style Checker already has one). In-world check of the M9.2 charts (Style Checker profile/help/social + GM Dashboard scene/disguise tabs) is the first M9.3 errand.
