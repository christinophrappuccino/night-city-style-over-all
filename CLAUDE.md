...
**M8 RELEASED (v0.1.0 live on GitHub; CLI-verified; in-Foundry fresh-world check + README screenshots still pending) — M9 IN PROGRESS: M9.1 DONE (gate 246/246), M9.2 (chart/transparency library) is next (see the M9 plan below).** M0–M7 are complete and verified in-world:
...
- **M9.2 — Visual component library (§25) + transparency standard (§19) — CURRENT:** radar · gauge · ring · fingerprint bar · sparkline + breakdown partial + info affordance + help/glossary. Built once, reused by every app. The engine data for all of it now exists (vibes.spokes is radar-ready).
