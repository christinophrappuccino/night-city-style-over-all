...
**M9.4 is CODE-COMPLETE except the AppV2 class-swap (2026-06-11, gate 303/303, all pushed): M9.4a tuning panel + scoring presets (`b9c091c`) · M9.4b public API + hooks (`d81c2c8`) · M9.4c SVG recolor service (`9dff68a`) · M9.4d = the documented path (`docs/appv2-migration.md`) — the class migration is DELIBERATELY sequenced AFTER the sweep (guide D2 escape hatch: don't confound port breakage with unverified-feature breakage). Also landed earlier same day: the polish pass (`a4415b4`+`9ec78b1`) and the macro terminal-language elevation (`677a26d`).** Template-level hierarchy judgment + macro-mine items (grade rings, portrait frames, voice microcopy) still ride the sweep findings. Next session, in order:
