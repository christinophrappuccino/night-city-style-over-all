...
**M8 RELEASED (v0.1.0 live on GitHub; CLI-verified; in-Foundry fresh-world check + README screenshots still pending) — M9 IN PROGRESS: M9.1 DONE (246/246) · M9.2 DONE (265/265; charts confirmed in-world, button-skin follow-up landed) · M9.3 IN PROGRESS: M9.3a (Style Checker headline + observer lens) and M9.3b (Wardrobe wearMode toggles + as-target-sees-it) CODE-COMPLETE (gate 278/278; in-world checks pending), M9.3c (GM Dashboard + GM Overrides) is next (see the M9 plan below).** M0–M7 are complete and verified in-world:
...
**M9.3 state — M9.3a + M9.3b DONE, M9.3c (GM Dashboard + GM Overrides) is next.** The chunk plan: **a)** Style Checker headline+lens ✓ · **b)** Wardrobe wearMode toggles + as-target-sees-it ✓ · **c)** GM Dashboard (GM Overrides UI §14.3 — pin reads on actor flags, clearly badged + consumed by the pipeline; layout pass; Help tab) · **d)** lookbook card §16.4 + player↔player reads §16.3 · **e)** GM Config (faction matrix editor §14.7, brand house pages §13) + Help tabs everywhere.
